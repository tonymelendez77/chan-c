...
# Match statuses that mean the worker has a pending offer
...
def _validate_twilio_signature(request: Request, form_body: dict) -> bool:
    ...
    if not settings.TWILIO_WEBHOOK_SECRET:
        # Skip validation in dev if no secret configured
        return True
...
async def incoming_sms(
    ...
    """Twilio webhook — receives incoming SMS from workers. No JWT required; validates Twilio signature instead."""
    ...
    # Strip +502 prefix to match stored phone format
    phone = From.replace("+502", "").replace("+", "").strip()
    ...
    # Parse reply first — we need to know if it's TRABAJAR before worker lookup matters
    reply = parse_worker_reply(Body)

    # --- TRABAJAR flow: new worker registration ---
    if reply == "TRABAJAR":
        await sms_service.log_inbound_sms(
            db,
        ...
        if worker is not None and worker.is_active:
            ...
            await sms_service.send_sms(
                db, worker.phone,
                "Hola. Gracias por tu interes en CHAN-C. En unos minutos te llamamos para una entrevista rapida. Ten listo tu DPI.",
                worker_id=worker.id,
            )
    ...
    if reply == "PAUSAR":
        ...
        if worker is None:
            ...
            await sms_service.send_sms(
                db, worker.phone,
                f"Ya tienes las ofertas pausadas {worker.full_name}. Escribe REANUDAR cuando estes disponible.",
                worker_id=worker.id,
            )
        else:
            worker.paused = True
            worker.is_available = False
            await sms_service.send_pause_confirmation(db, worker)
        await db.commit()
        return Response(content=str(twiml), media_type="text/xml")
    ...
    if reply == "REANUDAR":
        ...
        if worker is None:
            ...
            await sms_service.send_sms(
                db, worker.phone,
                f"Ya estas activo {worker.full_name}. Te avisamos cuando haya trabajo disponible.",
                worker_id=worker.id,
            )
        else:
            worker.paused = False
            ...
            await sms_service.send_resume_confirmation(db, worker)
        await db.commit()
        return Response(content=str(twiml), media_type="text/xml")

    # --- Existing flow: SI/NO/CONTRA or unknown ---
    if worker is None:
        twiml.message("No encontramos tu numero. Escribe TRABAJO para registrarte.")
        await sms_service.log_inbound_sms(
    ...
    # Find most recent pending match for this worker
    match_stmt = (
        select(Match)
    ...
    if reply == "SI":
        ...
        reply_msg = "Gracias. Te llamamos pronto para confirmar detalles."
        await sms_service.send_sms(
            db, worker.phone, reply_msg, worker_id=worker.id, match_id=match.id
        )
        ...
        match.worker_replied_at = datetime.now(timezone.utc)
        await sms_service.send_job_declined(db, worker)

    elif reply == "CONTRA":
        match.status = MatchStatus.pending_ai_call
        match.worker_reply = WorkerReply.contra
        match.worker_replied_at = datetime.now(timezone.utc)
        await sms_service.send_counteroffer_call_notice(db, worker)

    else:
        # Unknown reply — ask them to try again
        reply_msg = "No entendimos tu respuesta. Responde SI, NO o CONTRA."
        await sms_service.send_sms(
            db, worker.phone, reply_msg, worker_id=worker.id, match_id=match.id
        )
...
async def send_offer(
    ...
    """Admin endpoint — send a job offer SMS to the matched worker and update match status."""
    stmt = select(Match).where(Match.id == data.match_id)
    ...
    sid = await sms_service.send_job_offer(db, worker, match, job)

    match.status = MatchStatus.pending_worker
    match.worker_sms_sent_at = datetime.now(timezone.utc)

    await db.commit()
    return {"detail": "Job offer SMS sent", "twilio_sid": sid}
...
async def send_test(
    ...
    """Admin endpoint — send a test SMS to any phone number. For testing purposes only."""
    sid = await sms_service.send_sms(db, data.phone, data.message)
    await db.commit()
    return {"detail": "Test SMS sent", "twilio_sid": sid}
